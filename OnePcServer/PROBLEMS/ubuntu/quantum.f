...
c     .......... find both eigenvalues and eigenvectors ..........
   ...
   50 return
	...
	double precision function astar()
	astar=1.6*1d-19
	end

	double precision function v0()
	v0=0.1
	end
	...
	double precision function dl0()
	dl0=1d-10
	end

	double precision function bj(j,x)
      implicit double precision (a-h,o-z)
	factor1=(2.0*j+1.0)/(2.0*j)*coth((2.0*j+1.0)/(2.0*j)*x)
	factor2=1.0/(2.0*j)*coth(1.0/(2.0*j)*x)
	bj=factor1-factor2
	end

	double precision function mb()
	mb=5.788*1d-5
	end

	double precision function gmh()
	gmh=2.0
	end

	double precision function bt()
      implicit double precision (a-h,o-z)
CCC	Returns the expression bt=bext+bs
	parameter(bs=0.05)
	parameter(bext=0.6)
	bt=bext+bs
	end

	double precision function sz()
CCC	computes the sz expression
      implicit double precision (a-h,m,o-z)
	parameter (s=2.5)
	parameter (akb=8.617d-5)
	parameter (T=1.0)
	parameter (T0=40.0)
	parameter (j=2)
        arg=s * gmh()*mb()*bt()/(akb*(t+t0))
	sz=s*bj(j,arg)
	end

	double precision function geff()
      implicit double precision (a-h,m,o-z)
	parameter(g=-1.47)
	parameter(n0a=0.22)
        parameter(xmn=0.2)
        v1=sz()
        egeff=g-n0a*xmn*sz()/(mb()*bt())
        geff=egeff
	end

	double precision function vc(z)
       implicit double precision (a-h,o-z)
	parameter (v1=-0.105)
	parameter (v2=-0.166)
	parameter (v3=0.0)
	...
	double	precision function veff(z,sigma)
       implicit double precision (a-h,m,o-z)
	parameter(e=1.6*1d-19)
	parameter(fq=1d-2)
	veff=geff()*mb()*bt()*sigma-e*fq*z+vc(z)
	end

	double precision function vefftilde(z,sigma)
       implicit double precision (a-h,m,o-z)
	parameter(e=1.6*1d-19)
	parameter(fg=1d-3)
	factor1=geff()*mb()*bt()/(v0())*sigma
	factor2=e*fg*z/(astar()*v0())
	factor3=vc(z)/v0()
	vefftilde=factor1-factor2+factor3
	end

      subroutine nets(ann,fdann,sdann,r,u,w,v,nodes)
      implicit double precision (a-h,o-z)
      dimension u(nodes),w(nodes),v(nodes)
CC	ann The output of the neural network for the current input
CC	fdann The first derivative of the neural network for the current input
CC	sdann The second derivative of the neural network for the current input
      ann = 0
      fdann = 0
      sdann = 0.
      ...
      do 1 i = 1,nodes
        ...
        sdsig = fdsig*(1-2*sig)
        ann = ann + v(i)*sig
        fdann = fdann + w(i)*v(i)*fdsig
        sdann = sdann + w(i)**2*v(i)*sdsig
 1    continue
      end

      double precision function calcmass(z)
      implicit double precision (a-h,o-z)
      parameter (cmass0=9.109*1d-31)
      parameter (cmass1=0.10 * cmass0)
      parameter (cmass2=0.13 * cmass0)
      parameter (cmass3=0.096 * cmass0)
	if((z.ge.0).and.(z.lt.7)) then 
		calcmass=cmass1
	endif
        if((z.ge.7).and.(z.lt.15)) then 
		calcmass=cmass2
	endif
        if(z.ge.15) then
		 calcmass=cmass3
      ...
      parameter (nodes=10)
      parameter (d=1)
      parameter (zrange=70,nop = 100)
      parameter (mrange=50)
      parameter (iscount=mrange)
CCCC  joule 
      parameter (heta=1.054d-34)
CCCC   eV * second
CCC   parameter (heta=6.582d-16)
      dimension u(nodes),w(nodes),v(nodes) 
      dimension sigma(iscount)
      dimension m(mrange)
      dimension a(mrange,iscount)
      dimension b(mrange,iscount)
      dimension alfr(iscount)
      dimension alfi(iscount)
      dimension beta(iscount)
      parameter (n=(d+2)*nodes+mrange*iscount)
      dimension x(n)
      dimension zeta(mrange,iscount)
 ...
 2	continue
	...
	do 3 i=1,nop
	 z=i*zrange*1.0/nop
        call nets(y,fdy,sdy,z,u,w,v,nodes)

CC	matz=1
CC	call rgg(mrange,iscount,a,b,alfr,alfi,beta,matz,zeta,ierr)

	izcount=(d+2)*nodes+1
        do 6 im=1,mrange
	 do 7 is=1,iscount
          cmass=calcmass(z)*1d+31
	  value1=(1.054)**2/(2.0 *cmass) * 1d-7
          factor1=value1*sdy
          factor2=(vefftilde(z,sigma(is))-x(izcount))*y
          izcount=izcount+1
          if(abs(factor1).gt.1d+10) then
              funmin=1d+10
              return
          endif
	  tsum=tsum+(factor1+factor2)**2
 ...
 3	continue
	...
	funmin=tsum
      ...
      parameter (mrange=50)
      parameter (iscount=mrange)
        getdimension=(d+2)*nodes+mrange*iscount
      end

        subroutine getleftmargin(xl)
        parameter (nodes=10)
        parameter (d=1)
        parameter (mrange=50)
        parameter (iscount=mrange)
        parameter (mdim=(d+2)*nodes+mrange*iscount)
        integer i
        double precision xl(mdim)
        do 1 i=1,mdim
                xl(i)=-10
 1      continue
        end

        subroutine getrightmargin(xl)
        parameter (nodes=10)
        parameter (d=1)
        parameter (mrange=50)
        parameter (iscount=mrange)
 ...
 1      continue
      ...
      parameter (mrange=50)
      parameter (iscount=mrange)
      parameter (mdim=(id+2)*nodes+mrange*iscount)
      implicit double precision (a-h,o-z)
      dimension x(mdim),g(mdim)
      integer getdimension
      double precision funmin
      do 1 i=1,getdimension()
                eps=(1e-18)**(1.0/3.0)*max(1.0,abs(x(i)))
                x(i)=x(i)+eps
                v1=funmin(x)
                x(i)=x(i)-2.0*eps
                v2=funmin(x)
  ...
  1     continue
	...
	open(1,file='quantum.log')
	izcount=(d+2)*nodes+1
        do 61 im=1,mrange
		write(1,*) (x((im-1)*iscount+is),is=1,iscount)
 61 	continue
	close(1)
	done=1
	end
